/**
 * FILE 04 — sabai-bible/src/explore/utils/makeStoryVideo.js
 * ─────────────────────────────────────────────────────────────
 * PURPOSE:  100% client-side video renderer. Runs entirely in
 *           the browser — no server involvement after the
 *           story manifest has been fetched.
 *
 * HOW IT WORKS (high-level):
 *   1. Receives the story manifest from the API (scenes with
 *      base64 image data URLs + WAV audio data URL).
 *   2. Creates an AudioContext and decodes the WAV.
 *   3. Creates an off-screen <canvas> at 1280×720.
 *   4. Uses MediaRecorder + canvas.captureStream() to record
 *      the canvas as a video stream.
 *   5. Connects the decoded audio to a MediaStreamDestination
 *      and mixes it into the recording stream.
 *   6. A rAF loop draws each frame: intro card → scene images
 *      with Ken Burns zoom + crossfades → outro card.
 *   7. When the timeline ends, MediaRecorder.stop() is called
 *      and the collected chunks are assembled into a Blob.
 *   8. Returns a Blob (WebM or MP4 depending on browser).
 *
 * KEY CONSTRAINTS:
 *   • AudioContext must be created SYNCHRONOUSLY in the same
 *     call stack as the user gesture (Chrome autoplay policy).
 *     Any await before `new AudioContext()` causes the context
 *     to be created in a suspended state and audio is silent.
 *   • Keep the browser tab visible during recording — Chrome
 *     throttles rAF in background tabs. The setInterval
 *     fallback handles this but quality may drop.
 *   • MediaRecorder is not available in all browsers. Use
 *     isVideoSupported() before calling makeStoryVideo().
 *
 * EXPORTS:
 *   makeStoryVideo(story, onProgress)  → Promise<Blob>
 *   downloadBlob(blob, filename)
 *   saveVideoLocally(eventId, blob)    → IndexedDB
 *   loadVideoLocally(eventId)          → IndexedDB
 *   isVideoSupported()                 → boolean
 *   estimateVideoDuration(story)       → number (seconds)
 *
 * RUNTIME:  Browser (Chrome/Edge recommended; Firefox supported)
 */
...
// ── Frame renderer ─────────────────────────────────────────────────────────────
// Called on every animation frame. Draws the current state of the timeline
// onto the canvas based on `elapsed` seconds since recording started.
function drawFrame(ctx2d, state, story, images) {
  ...
  if (seg.type === 'intro') {
    ...
    // ── Scene with Ken Burns + crossfade ─────────────────────────────────────
    ...
    // Darkening gradient at bottom (makes caption readable)
...
// ── MAIN EXPORT ───────────────────────────────────────────────────────────────
/**
 * Renders the story manifest into a video Blob.
 *
 * @param {object}   story      - manifest returned by /api/events/:id/story
 * @param {function} onProgress - called with a float 0..1
 * @returns {Promise<Blob>}     - WebM or MP4 depending on browser
 */
export async function makeStoryVideo(story, onProgress) {
  if (!isVideoSupported()) {
    throw new Error('Your browser does not support canvas video capture. Try Chrome or Edge.');
  }

  // CRITICAL: Create AudioContext SYNCHRONOUSLY before any await.
  // Chrome's autoplay policy suspends AudioContexts that are created after
  // an asynchronous gap from the user-gesture event handler.
  ...
  // Resume in case the browser pre-suspended the context (Safari)
  ...
  // Build timeline: intro + N scenes + outro
  const rawDurs    = (story.scenes || []).map((s) => Number(s.durationSec) || 7);
  const sceneTotal = rawDurs.reduce((a, b) => a + b, 0);
  // Scale scene durations to match audio length so video stays in sync
  ...
  if (audioBuffer) {
    ...
    srcNode.start(audioCtx.currentTime + INTRO_S); // delay audio to match intro card
  ...
  return new Promise((resolve, reject) => {
    ...
    // Safety: force-stop if animation loop hangs
    safetyTimer = setTimeout(() => {
      if (recorder.state !== 'inactive') recorder.stop();
    }, (totalDur + 10) * 1000);

    recorder.start(500); // emit data chunks every 500ms
    ...
    // setInterval backup for background tabs (rAF freezes when tab is not active)
...
// Saves the finished Blob so users can reload the page without re-rendering.
