...
function NavRail({ workspace, onWorkspace }) {
  return (
    <aside className="nav-rail">
      ...
      <button type="button" title="Journey map" className={workspace === 'journey' ? 'active' : ''} onClick={() => onWorkspace('journey')}>
        <Map size={20} />
      </button>
      <button type="button" title="Bible Timeline" className={workspace === 'timeline' ? 'active' : ''} onClick={() => onWorkspace('timeline')}>
        <CalendarDays size={20} />
      </button>
      <button type="button" title="People" className={workspace === 'journey' ? '' : ''} onClick={() => onWorkspace('journey')}>
        <Users size={20} />
      </button>
      <button type="button" title="Family lineage" className={workspace === 'journey' ? '' : ''} onClick={() => onWorkspace('journey')}>
        <TreePine size={20} />
      </button>
      <button type="button" title="Scripture" onClick={() => onWorkspace('journey')}>
        <BookOpen size={20} />
      </button>
      <div className="rail-spacer" />
      <button type="button" title="Back to map" onClick={() => onWorkspace('journey')}>
        <ChevronLeft size={22} />
      </button>
    </aside>
  );
...
function EventList({ events, selected, onSelect, query, setQuery, activeEra }) {
  ...
  const filtered = useMemo(() => {
    ...
    return events.filter((event) => {
      const matchesQuery = !q || [event.title, event.era, event.timelineDate, event.mapLocation, ...(event.references || []), ...(event.mainPeople || [])]
        .join(' ')
        .toLowerCase()
        .includes(q);
  ...
  useEffect(() => {
    const node = listRef.current?.querySelector(`button[data-event-id="${selected.id}"]`);
        ...
        {filtered.map((event) => (
          ...
          >
            ...
            <span className="event-copy">
              ...
              <small>{event.references?.[0]} · {event.timelineDate}</small>
            </span>
  ...
  return (
    <header className="topbar">
      <div>
        ...
        <span>Interactive atlas: real tiles, story markers, and optional atlas routes</span>
      </div>
...
function RightPanel({ selected, content, lineageMatchSummary, lineageFocusLabels, onOpenLineageTree, onViewAllPeople }) {
  ...
  useEffect(() => {
    setPersonPerspectiveName(null);
  }, [selected.id]);

  useEffect(() => {
    if (!personPerspectiveName) return;
    const onKey = (e) => {
      if (e.key === 'Escape') setPersonPerspectiveName(null);
    };
  ...
  const perspectiveEntry =
    personPerspectiveName != null ? getPersonPerspectiveEntry(selected.id, personPerspectiveName) : null;
  const perspectiveFallback =
    personPerspectiveName != null
  ...
  return (
    <aside className="right-panel">
      <section className="panel lineage-panel">
        <div className="lineage-panel__head">
          <div className="panel-title-row lineage-panel__title-row">
            <div>
              <h2>Family thread</h2>
              <p>How this story sits in Scripture’s big story</p>
            </div>
            <button type="button" className="icon-button" title="Open full family tree" onClick={onOpenLineageTree}>
              <TreePine size={17} />
            </button>
          </div>
        </div>
        ...
        {lineageFocusLabels.length > 0 ? (
          <div className="lineage-panel__focus-track">
            <span className="lineage-panel__focus-heading">
              <Sparkles size={14} aria-hidden /> Tree match for this event
            </span>
            <div className="lineage-panel__focus-pills">
              {lineageFocusLabels.slice(0, 5).map((name, i) => (
                <span key={`${name}-${i}`} className="lineage-panel__pill">
                  {name}
                </span>
              ))}
              {lineageFocusLabels.length > 5 ? <span className="lineage-panel__pill lineage-panel__pill--more">+{lineageFocusLabels.length - 5}</span> : null}
            </div>
          </div>
        ) : null}

        {threadPreview ? (
          <p className="lineage-panel__thread-preview" title={thread.join(' → ')}>
        ...
        <div className="lineage-vine">
          {(selected.lineageConnection || []).length ? (
            (selected.lineageConnection || []).slice(0, 8).map((name, index, arr) => (
              <div
                key={`${name}-${index}`}
                className={`lineage-step ${index === arr.length - 1 ? 'lineage-step--active' : ''}`}
              >
                <div className="lineage-step__avatar-col">
                  {index > 0 ? <span className="lineage-step__stem lineage-step__stem--up" aria-hidden /> : null}
                  <div className="lineage-step__portrait-ring">
                    <div className="portrait">{initials(name)}</div>
                  </div>
                  {index < arr.length - 1 ? <span className="lineage-step__stem lineage-step__stem--down" aria-hidden /> : null}
                </div>
        ...
        <div className="lineage-panel__cta-card">
          <div className="lineage-panel__cta-icon" aria-hidden>
            <TreePine size={22} />
          </div>
          <div className="lineage-panel__cta-copy">
            ...
            <p>
              {lineageMatchSummary
                ? <>Poster view highlights <em>{lineageMatchSummary}</em> on the messianic line.</>
                : <>Open the illustrated poster or the complete graph—names light up where this event connects.</>}
            </p>
          </div>
          <button type="button" className="primary lineage-panel__tree-cta" onClick={onOpenLineageTree}>
            Open tree
      ...
      <section className="panel people-panel">
        <div className="panel-title-row">
          <div>
            ...
            <p>Tap someone to read their angle on this event</p>
          </div>
          <button type="button" className="icon-button" aria-label="People involved">
            <Users size={17} />
          </button>
        </div>
        <div className="people-list">
          {people.map((name) => (
            <button
              ...
              onClick={() => setPersonPerspectiveName(name)}
            >
              ...
              <span className="person-row__hint" aria-hidden>
                <MessageCircle size={16} strokeWidth={2} />
              </span>
            </button>
          ))}
        </div>
        <button type="button" className="link-button" onClick={onViewAllPeople}>
      ...
      {personPerspectiveName ? (
        <div
          className="person-perspective-root"
          role="dialog"
          aria-modal="true"
          aria-labelledby="person-perspective-title"
        >
          <button
            type="button"
            className="person-perspective-backdrop"
            aria-label="Close"
            onClick={() => setPersonPerspectiveName(null)}
          />
          <div className="person-perspective-dialog">
            <button
              type="button"
              className="icon-button person-perspective__close"
              aria-label="Close"
              onClick={() => setPersonPerspectiveName(null)}
            >
              <X size={20} />
            ...
            {selected.references?.[0] ? (
              <p className="person-perspective__ref">
                <BookOpen size={14} aria-hidden /> {selected.references[0]}
              </p>
            ) : null}
            <div className="person-perspective__body">
              <p>{perspectiveEntry?.perspective ?? perspectiveFallback}</p>
            </div>
            <button type="button" className="primary person-perspective__done" onClick={() => setPersonPerspectiveName(null)}>
              Close
            </button>
          </div>
        </div>
...
function Timeline({ activeEra, setActiveEra, scrollFocusPulse = 0 }) {
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      const active = track?.querySelector('.timeline-node.active');
  ...
  return (
    <section ref={sectionRef} id="bible-timeline" className="timeline panel">
      <div className="timeline-heading">
        ...
        <strong>Bible Timeline</strong>
      </div>
      <button type="button" className="round timeline-nav timeline-nav--prev" aria-label="Scroll timeline earlier" onClick={() => scrollBy(-160)}>
        <ChevronLeft size={18} />
      </button>
      ...
      <button type="button" className="round timeline-nav timeline-nav--next" aria-label="Scroll timeline later" onClick={() => scrollBy(160)}>
...
function StoryCacheChoiceModal({ meta, eventLabel, busy, enterConfirm, onCloseNotNow, onLoadSaved, onConfirmReplace }) {
  ...
  useEffect(() => {
    setStep(enterConfirm ? 'confirm' : 'pick');
  }, [meta?.eventId, enterConfirm]);

  useEffect(() => {
    const onKey = (e) => {
      if (e.key === 'Escape' && !busy) onCloseNotNow();
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [busy, onCloseNotNow]);
  ...
  return (
    <div className="story-cache-modal-root" role="dialog" aria-modal="true" aria-labelledby="story-cache-title">
      ...
      <div className="story-cache-modal">
        <button type="button" className="story-cache-modal__close" aria-label="Close" onClick={() => !busy && onCloseNotNow()}>
          <X size={20} />
        </button>
        {step === 'pick' ? (
          <>
            ...
            <p className="story-cache-modal__lede">
              <strong>{eventLabel}</strong> already has generated scenes and narration on this server.
            </p>
            <ul className="story-cache-modal__facts">
              <li>Saved: {dateStr}</li>
              <li>Scenes: {meta.sceneCount}</li>
            ...
            <div className="story-cache-modal__actions">
              <button type="button" className="primary" disabled={busy} onClick={onLoadSaved}>
                {busy ? 'Loading…' : 'Load saved story'}
              </button>
              <button type="button" className="secondary" disabled={busy} onClick={() => setStep('confirm')}>
                <Wand2 size={16} /> Create new version
              </button>
              <button type="button" className="story-cache-modal__ghost" disabled={busy} onClick={onCloseNotNow}>
                Not now
              </button>
            </div>
          </>
          ...
          <>
            ...
            <p className="story-cache-modal__lede">
              A new run will call Gemini again and overwrite images and narration for <strong>{eventLabel}</strong>. The current cache will be lost.
            </p>
            <div className="story-cache-modal__actions">
              <button type="button" className="secondary" disabled={busy} onClick={() => setStep('pick')}>
                Back
              </button>
              <button type="button" className="primary danger" disabled={busy} onClick={onConfirmReplace}>
                {busy ? 'Generating…' : 'Yes, replace cache'}
              </button>
            </div>
          </>
        )}
...
function StoryPlayer({
  ...
  useEffect(() => {
    ...
    const onFsChange = () => {
      if (!document.fullscreenElement) setVideoCinema(false);
    };
  ...
  const downloadStoryMp4 = useCallback(async () => {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        throw new Error(errJson.error || `Download failed (${r.status})`);
      }
      const blob = await r.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${id}-story.mp4`;
      a.rel = 'noopener';
      ...
      window.alert(e?.message || 'Could not build the video file.');
    } finally {
      setMp4Busy(false);
    }
  ...
  const onAudioTime = (e) => {
    ...
    if (d && Number.isFinite(d)) {
      setAudioDuration(d);
      setAudioProgress((el.currentTime / d) * 100);
    }
  ...
  return (
    <section ref={sectionRef} className={`player panel story-player${videoCinema ? ' story-player--cinema' : ''}`}>
      {videoCinema && story && (
        <button type="button" className="story-player__cinema-close" onClick={exitCinema} aria-label="Exit fullscreen">
          <X size={22} />
        </button>
      )}
      {story && (
        <figure className={`story-theater${loading ? ' story-theater--busy' : ''}`}>
          <div className="story-theater__frame">
            {imgSrc ? (
              <img src={imgSrc} alt={scene?.title || story.title} />
            ) : (
      ...
      <div className="player-toolbar">
        <div className="player-left">
          ...
          <div>
            ...
            <p>Gemini builds illustrated scenes and narration. Your story plays here and on the map.</p>
          </div>
        </div>
        {story ? (
          <div className="player-actions">
            <button type="button" className="primary" onClick={toggle} disabled={!audioSrc}>
              {playing ? <Pause size={17} /> : <Play size={17} />} {audioSrc ? 'Play narration' : 'No audio'}
            </button>
            <audio
              ref={audioRef}
              ...
              onLoadedMetadata={(e) => {
                ...
                if (d && Number.isFinite(d)) setAudioDuration(d);
              }}
            ...
            />
            <div className="scene-stepper" role="tablist" aria-label="Scenes">
            ...
            <button type="button" className="secondary story-cinema-toggle" onClick={videoCinema ? exitCinema : enterCinema} title={videoCinema ? 'Exit fullscreen' : 'Fullscreen video'}>
              {videoCinema ? <Minimize2 size={17} /> : <Maximize2 size={17} />}
              {videoCinema ? ' Exit' : ' Fullscreen'}
            </button>
            <button
              type="button"
              className="secondary story-download-mp4"
              onClick={downloadStoryMp4}
              disabled={loading || mp4Busy || !audioSrc || !(story?.eventId || exportEventId)}
              title="Download one MP4 with scene images, crossfades, and narration audio (server needs ffmpeg)"
            >
              {mp4Busy ? <Loader2 size={17} className="story-download-mp4__spin" aria-hidden /> : <Download size={17} aria-hidden />}
              {mp4Busy ? ' Preparing…' : ' Download MP4'}
            </button>
          ...
          <div className="story-cache-inline">
            <div className="story-cache-inline__row">
              <HardDrive size={20} className="story-cache-inline__icon" aria-hidden />
              <div>
                ...
                <p className="story-cache-inline__meta">
                  {cacheBanner.generatedAt
                    ...
                    : 'Unknown date'}
                  {' · '}
                  {cacheBanner.sceneCount} scene{cacheBanner.sceneCount === 1 ? '' : 's'}
                  {cacheBanner.hasAudio ? ' · With narration' : ''}
                </p>
              </div>
            </div>
            <div className="story-cache-inline__actions">
              <button type="button" className="primary" onClick={onLoadCachedInline} disabled={loading}>
                {loading ? 'Loading…' : 'Load saved story'}
              </button>
              <button type="button" className="secondary" onClick={onOpenReplaceModal} disabled={loading}>
                <Wand2 size={15} /> New version
              </button>
              <button type="button" className="story-cache-inline__dismiss" onClick={onDismissCacheBanner}>
                Hide
              </button>
            </div>
          </div>
          ...
          <button type="button" className="primary" onClick={onGenerate} disabled={loading}>
            {loading ? 'Creating story…' : 'Create story video'}
          </button>
        )}
      </div>
    </section>
    ...
    <section className="content-drawer panel">
      <div className="drawer-header">
        <div>
          ...
          <p>{selected.references?.join(', ')} · {selected.timelineDate} · {selected.mapLocation}</p>
        </div>
      ...
      <div className="drawer-grid">
        <article>
          <h4>Event Details</h4>
          <p>{content?.teachingSummary || selected.details}</p>
        </article>
        <article>
          <h4>Lesson</h4>
          <p>{content?.applicationLesson || selected.lesson}</p>
        </article>
        <article>
          <h4>Map Route</h4>
          <p>{content?.mapExplanation || `${selected.title} is mapped around ${selected.mapLocation}.`}</p>
        </article>
        <article>
          <h4>Video Idea</h4>
          <p>{selected.videoIdea}</p>
        </article>
      </div>
    </section>
  );
...
export default function App() {
  ...
  /** When opening from "View All People", jump straight to the full graph (all nodes). */
  ...
  const handleFocusEventInTimeline = useCallback(
    (eventId) => {
      ...
      setTimelineScrollPulse((n) => n + 1);
    },
    [events]
  );

  const handleTimelineOpenMap = useCallback((mapEventId) => {
    if (!mapEventId) return;
    setSelectedId(mapEventId);
    setWorkspace('journey');
  }, []);

  const handleTimelineOpenLineage = useCallback((mapEventId) => {
    if (!mapEventId) return;
    setSelectedId(mapEventId);
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/events`).then((r) => r.json()).then(setEvents).catch(() => setEvents(localEvents));
  }, []);

  const selected = useMemo(() => events.find((e) => e.id === selectedId) || events[0], [events, selectedId]);

  const { peopleById: lineagePeopleById, indexed: lineageIndexed } = useMemo(
    () => buildLineageIndex(lineageTree.people),
    []
  );

  const lineageTargetIds = useMemo(
    () => matchEventToLineagePersonIds(selected, lineagePeopleById, lineageIndexed),
    [selected, lineagePeopleById, lineageIndexed]
  );

  const lineageHighlightNodes = useMemo(
    () => highlightNodesForTargets(lineageTargetIds, lineagePeopleById, lineageTree.metadata?.rootId || 'adam'),
    [lineageTargetIds, lineagePeopleById]
  );

  const lineageHighlightEdges = useMemo(
    () => highlightEdgesForNodes(lineageHighlightNodes, lineageTree.relationships || []),
    [lineageHighlightNodes]
  );

  const lineageMatchSummary = useMemo(() => {
    if (!lineageTargetIds.length) return '';
    return lineageTargetIds
      .map((id) => lineagePeopleById.get(id)?.displayName || lineagePeopleById.get(id)?.name || id)
      .join(' · ');
  }, [lineageTargetIds, lineagePeopleById]);

  const lineageFocusLabels = useMemo(() => {
    if (!lineageTargetIds.length) return [];
    return lineageTargetIds.map(
      (id) => lineagePeopleById.get(id)?.displayName || lineagePeopleById.get(id)?.name || id
    );
  }, [lineageTargetIds, lineagePeopleById]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (ac.signal.aborted || !meta.cached) return;
        setStoryCacheModal(meta);
      } catch {
  ...
  const loadCachedStory = async () => {
    ...
    try {
      ...
      setStoryCacheBanner(null);
      requestAnimationFrame(() => {
        storyPlayerRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      });
    } catch (err) {
      console.error(err);
  ...
  const handleCloseStoryCacheModal = () => {
    if (storyCacheModal) {
      const { enterConfirm: _e, ...snap } = storyCacheModal;
      setStoryCacheBanner({ ...snap, cached: true });
    }
    setStoryCacheModal(null);
  };

  const handleOpenReplaceFromBanner = () => {
    if (!storyCacheBanner) return;
    setStoryCacheModal({ ...storyCacheBanner, enterConfirm: true });
    setStoryCacheBanner(null);
  };
  ...
  const generateStory = async ({ force = false } = {}) => {
    setStoryLoading(true);
    try {
      const response = await fetch(`${API_BASE}/api/events/${selected.id}/story`, {
        method: 'POST',
      ...
      console.error(err);
      window.alert(err.message || 'Story generation failed.');
    } finally {
      setStoryLoading(false);
    }
  };
  ...
  return (
    <div className="app-shell">
      ...
      <div className="app-main">
        ...
        {import.meta.env.DEV && spriteDebug ? <SpriteDebugGrid /> : null}
        {lineageTreeOpen && (
          <LineageTreeModal
            open={lineageTreeOpen}
            onClose={() => {
              setLineageTreeOpen(false);
              setLineageTreeEntry('default');
            }}
            tree={lineageTree}
        ...
        {storyCacheModal && (
          <StoryCacheChoiceModal
            meta={storyCacheModal}
            enterConfirm={Boolean(storyCacheModal.enterConfirm)}
            eventLabel={selected.title}
            busy={storyLoading}
            onCloseNotNow={handleCloseStoryCacheModal}
            onLoadSaved={loadCachedStory}
            ...
            <div className="workspace">
              ...
              <div className="center-stack">
                <JourneyMap
                  ...
                  onFocusInTimeline={handleFocusEventInTimeline}
              ...
              <RightPanel
                selected={selected}
                content={content}
                lineageMatchSummary={lineageMatchSummary}
                ...
                onViewAllPeople={() => {
                  setLineageTreeEntry('fullMap');
                  setLineageTreeOpen(true);
                }}
              />
            </div>
            <StoryPlayer
              sectionRef={storyPlayerRef}
              story={story}
              loading={storyLoading}
              onGenerate={() => generateStory()}
              onRegenerate={() => generateStory({ force: true })}
              ...
              onOpenReplaceModal={handleOpenReplaceFromBanner}
